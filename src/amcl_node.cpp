...
public:
    ...
    // Save current pose in a static variable. By Jiaheng.
    static pf_vector_t currentPose_;
...
private:
    ...
    // Pose-generating function used to uniformly distribute particles near the robot.
    static pf_vector_t nearbyPoseGenerator(void *arg);
    ...
    bool nearbyLocalizationCallback(std_srvs::Empty::Request &req,
                                    std_srvs::Empty::Response &res);
    ...
    ros::ServiceServer near_loc_srv_;
...
pf_vector_t AmclNode::currentPose_;
...
AmclNode::AmclNode() :
    ...
    updatePoseFromServer(); // 这里我只看到了对初始pose的赋值和检查是否nan。 28/02 Update: add static variable currentPose_.

    cloud_pub_interval.fromSec(1.0);
    tfb_ = new tf::TransformBroadcaster();
    tf_ = new TransformListenerWrapper(); //这里是tf的listerner，作者定义的一个结构体。 By Jiaheng.
    ...
    global_loc_srv_ = nh_.advertiseService("global_localization",
                                           ...
                                           this);
    near_loc_srv_ = nh_.advertiseService("nearby_localization",
                                         &AmclNode::nearbyLocalizationCallback,
                                         this);
    ...
    laser_scan_filter_ =
            new tf::MessageFilter<sensor_msgs::LaserScan>(*laser_scan_sub_,
                                                          *tf_,
                                                          odom_frame_id_,
                                                          100); // Transform laser to odom frame.
    ...
    m_force_update = true;// change to true to force updating. By Jiaheng.
...
void AmclNode::updatePoseFromServer() {
    ...
    AmclNode::currentPose_.v[0] = 0.0;
    AmclNode::currentPose_.v[1] = 0.0;
    AmclNode::currentPose_.v[2] = 0.0;
    ...
    if (!std::isnan(tmp_pos)) {
        ...
        AmclNode::currentPose_.v[0] = tmp_pos;
    } else
    ...
    if (!std::isnan(tmp_pos)) {
        ...
        AmclNode::currentPose_.v[1] = tmp_pos;
    } else
    ...
    if (!std::isnan(tmp_pos)) {
        ...
        AmclNode::currentPose_.v[2] = tmp_pos;
    } else
...
AmclNode::convertMap(const nav_msgs::OccupancyGrid &map_msg) {
    ...
    // put the origin of map in the center. This is in global frame. By Jiaheng.
    ...
    for (int i = 0; i < map->size_x * map->size_y; i++) {
        if (map_msg.data[i] == 0) // map_msg.data is in row-major order. The origin is lower-left corner. By Jiaheng.
...
// Add a uniformly distribute pose generator near the robot. By Jiaheng.
// TODO: finish this part.
pf_vector_t
AmclNode::nearbyPoseGenerator(void *arg) {
    map_t *map = (map_t *) arg;
...
// re-localization near the robot
bool
AmclNode::nearbyLocalizationCallback(std_srvs::Empty::Request &req,
                                     std_srvs::Empty::Response &res) {
    if (map_ == NULL) {
        return true;
    }
    boost::recursive_mutex::scoped_lock gl(configuration_mutex_);
    ROS_INFO("Initializing with uniform distribution near the robot");
    pf_init_model(pf_, (pf_init_model_fn_t) AmclNode::nearbyPoseGenerator,
                  (void *) map_);
    ROS_INFO("Nearby initialisation done!");
    pf_init_ = false;
    return true;
}
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan) {
    ...
    if (frame_to_laser_.find(laser_scan->header.frame_id) ==
        frame_to_laser_.end()) { //frame_to_laser_是map类，find是迭代容器，如果容器为空，那么就往下进行。
    ...
    if (!pf_init_) {
        ...
        odom_->UpdateAction(pf_,
                            (AMCLSensorData *) &odata); //* This is the most important one in amcl_odom.cpp. BY Jiaheng.
...
//
            AmclNode::currentPose_.v[0] = hyps[max_weight_hyp].pf_pose_mean.v[0];
            AmclNode::currentPose_.v[1] = hyps[max_weight_hyp].pf_pose_mean.v[1];
            AmclNode::currentPose_.v[2] = hyps[max_weight_hyp].pf_pose_mean.v[2];
